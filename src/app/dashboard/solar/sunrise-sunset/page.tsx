...
import { solarCalc, SunTimesResult } from "@/lib/api/solar";
...
export default function SunriseSunsetPage() {
  ...
  const [result, setResult] = useState<SunTimesResult | null>(null);
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const res = await solarCalc.calculateSunriseSunset({
        latitude: lat,
        longitude: lon,
        date: date,
        timezone: timezone,
      ...
      trackEvent({
        ...
        resource: "/dashboard/solar/sunrise-sunset",
      });
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Sunrise & Sunset</h1>
        <p className="text-muted-foreground mt-2">
          Calculate sunrise, sunset, and day length for a given location and date. Handles polar day/night conditions.
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Sun times details.</CardDescription>
          </CardHeader>
            ...
            {result ? (
              <Table>
                ...
                <TableBody>
                  ...
                  <TableRow><TableCell>Sunrise (Local Time)</TableCell><TableCell className="text-right">{result.sunrise ? new Date(result.sunrise).toLocaleTimeString([], {timeZone: result.timezone, hour: '2-digit', minute:'2-digit'}) : "Polar Night/Day"}</TableCell></TableRow>
                  <TableRow><TableCell>Sunset (Local Time)</TableCell><TableCell className="text-right">{result.sunset ? new Date(result.sunset).toLocaleTimeString([], {timeZone: result.timezone, hour: '2-digit', minute:'2-digit'}) : "Polar Night/Day"}</TableCell></TableRow>
                  <TableRow><TableCell>Day Length</TableCell><TableCell className="text-right">{(result.day_length_minutes / 60).toFixed(2)} hours</TableCell></TableRow>
                </TableBody>
              </Table>
            ) : (
