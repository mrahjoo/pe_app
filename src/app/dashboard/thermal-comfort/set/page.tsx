...
import { comfortApi, SetTmpResponse } from "@/lib/api/thermalcomfort";
import { Calculator } from "lucide-react";

export default function SetCalculatorPage() {
  const [tdb, setTdb] = useState<string>("25");
  const [tr, setTr] = useState<string>("25");
  const [v, setV] = useState<string>("0.15");
  const [rh, setRh] = useState<string>("50");
  const [met, setMet] = useState<string>("1.2");
  const [clo, setClo] = useState<string>("0.5");
  
  const [result, setResult] = useState<SetTmpResponse | null>(null);
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const res = await comfortApi.set(parsed);
      setResult(res);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Standard Effective Temperature (SET)</h1>
        <p className="text-muted-foreground mt-2">
          Calculate the equivalent temperature combining air, radiant, humidity, and air speed effects.
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Environmental & Personal Factors</CardTitle>
            <CardDescription>Enter physical parameters (SI Units).</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button onClick={handleCalculate} className="w-full mt-4" disabled={loading}>
              ...
              {loading ? "Calculating..." : "Evaluate SET"}
            </Button>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Equivalent Temperature</CardTitle>
            <CardDescription>Standard Effective Temperature (SET)</CardDescription>
          </CardHeader>
          <CardContent>
            {result ? (
              <div className="flex flex-col items-center justify-center space-y-6 p-6 border rounded-lg bg-muted/20 h-full">
                <div className="text-center">
                  <div className="text-sm font-semibold text-muted-foreground uppercase mb-1">SET</div>
                  <div className="text-5xl font-bold text-primary">
                    {Number(result.set).toFixed(1)} <span className="text-3xl text-muted-foreground">°C</span>
                  </div>
                </div>
              </div>
            ) : (
